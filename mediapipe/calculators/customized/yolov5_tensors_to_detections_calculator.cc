...
class Yolov5TensorsToDetectionsCalculator : public Node {
 public:
 ...
 private:
  ...
  absl::Status DecodeYoloBoxes(const float* raw_boxes,
...
MEDIAPIPE_REGISTER_NODE(Yolov5TensorsToDetectionsCalculator);

absl::Status Yolov5TensorsToDetectionsCalculator::UpdateContract(
...
absl::Status Yolov5TensorsToDetectionsCalculator::Open(CalculatorContext* cc) {
...
absl::Status Yolov5TensorsToDetectionsCalculator::Process(CalculatorContext* cc) {
...
absl::Status Yolov5TensorsToDetectionsCalculator::ProcessCPU(
  ...
  if (input_tensors.size() == 2)
  {
    ...
    MP_RETURN_IF_ERROR(DecodeYoloBoxes(raw_boxes, &boxes));
...
absl::Status Yolov5TensorsToDetectionsCalculator::ProcessGPU(
    CalculatorContext* cc, std::vector<Detection>* output_detections) {
  const auto& input_tensors = *kInTensors(cc);
...
absl::Status Yolov5TensorsToDetectionsCalculator::Close(CalculatorContext* cc) {
#ifndef MEDIAPIPE_DISABLE_GL_COMPUTE
...
absl::Status Yolov5TensorsToDetectionsCalculator::LoadOptions(CalculatorContext* cc) {
...
absl::Status Yolov5TensorsToDetectionsCalculator::DecodeYoloBoxes(
      const float* raw_boxes, std::vector<float>* boxes) {
...
absl::Status Yolov5TensorsToDetectionsCalculator::ConvertToDetections(
...
Detection Yolov5TensorsToDetectionsCalculator::ConvertToDetection(
...
absl::Status Yolov5TensorsToDetectionsCalculator::GpuInit(CalculatorContext* cc) {
...
kernel void decodeKernel(
  ...
  x_center = x_center / scale.x;
  y_center = y_center / scale.y;
...
bool Yolov5TensorsToDetectionsCalculator::IsClassIndexAllowed(int class_index) {
